(function ($, window, document, undefined) {
	...
	var Navigation = {
		...
		bindEvents: function (self) {
			...
			self.$nav.on('click', self.subMenu + ' > a', function (event) {
				...
				// If not, then there is no use in toggling the submenu
		...
		openSubMenu: function (self, li) {
			...
			li.siblings(self.subMenu).removeClass('nav-active')
				// Close all sibling submenu's
				.children('ul').slideUp(self.slideSpeed, function () {
					$(this).clearQueue();
				})
				// Then deactivate their submenu's
		...
		closeSubMenu: function (self, li) {
			...
			li.removeClass('nav-active')
				...
				// Deactivate submenu's of the clicked submenu
				.find('.nav-active').removeClass('nav-active')
				// And close them
				.children('ul').slideUp(self.slideSpeed, function () {
					$(this).clearQueue();
		...
		// Reset nav when we switch from mobile to desktop mode and vice versa
		...
		// If .nav-button is visible then we're in mobile mode!
	...
	$.fn.nav.options = {
		...
		// submenu selector (<li>)
		subMenu: '.nav-submenu',
		// Open submenu's on mouse over when not in mobile mode
		mouseOver: true,
		// When clicking/touching a submenu link, it will open the submenu...
		// Not disabling the links will make submenu's unreachable on touch devices!
		// A link with [href="#"] will always be disabled, regardless of this setting.
		// Disable the actual link in a particular mode: always|never|mobile|desktop
		disableSubMenuLink: 'always',
		// How fast should a submenu open/close? (ms)
		slideSpeed: 300
	};
